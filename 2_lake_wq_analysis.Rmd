...
```
As mean secchi increases the mean chlorophyll A observed decreases.

## Why might this be the case? 
When the water is more clear, you can see the secchi disk at greater depths.
...
## Your code here
site_sf <- st_as_sf(site_count) %>%
  st_transform(.,crs=4326) %>%
  select(-(10:18))

site_count_states <- site_sf %>%
  st_join(.,states) %>%
  select(-(14:18),-(20:25)) # remove some extra columns
...
obs_per_state <- site_count_states %>%
  group_by(name) %>%
  group_by(lagoslakeid,add=TRUE) %>%
  summarise(xcount=max(count)) %>%
  summarise(st_count=sum(xcount)) %>%
...
```{r}
## Your code here
# join secchi data with spatial lakes
secchi_spatial_200 <- chla_secchi_200 %>%
  inner_join(.,spatial_lakes,by="lagoslakeid") %>%
  st_as_sf(.)

mapview(secchi_spatial_200)
